...
| Push version tag such as `v1.0.0` | Yes | Yes, after all checks pass |
...
trivy image \
  ...
  --severity CRITICAL \
  --exit-code 1 \
  ghcr.io/ahsanjamil583/ci-cd-pipelines:v1.0.0
```
...
This job runs only for semantic version tags such as `v1.0.0`.
...
It runs only when a version tag such as `v1.0.0` is pushed.
